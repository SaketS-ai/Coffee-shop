...
export const AuthGate: React.FC<AuthGateProps> = ({
  ...
  if (user && allowedRoles.includes(user.role)) {
    return (
      <div className="flex items-center justify-between bg-[#FFF8F0] border border-[#8C5A3C]/20 rounded-2xl px-4 py-2.5 text-xs">
        <span className="text-[#4B2E2B] font-bold">
          Signed in as {user.name} ({user.role})
        </span>
        <button
          onClick={logout}
          className="flex items-center space-x-1 text-[#8C5A3C] hover:text-[#4B2E2B] font-bold"
        >
