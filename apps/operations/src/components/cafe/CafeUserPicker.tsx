...
export const CafeUserPicker: React.FC<CafeUserPickerProps> = ({ onSuccess }) => {
  ...
  // Fetch all partner cafes dynamically from PostgreSQL
  useEffect(() => {
    ...
    const loadCafes = async () => {
      try {
        ...
        if (!isCancelled) {
          setFetchError(err.message || 'Unable to load partner cafes from database.');
        }
      } finally {
        if (!isCancelled) {
          setIsLoading(false);
  ...
  // Filter cafes by name, neighborhood, or address
  ...
  const handleVerifyPin = async () => {
    ...
    if (!targetPin || targetPin.length < 4) {
      setPinError('Please enter the 4-digit cafe PIN.');
      return;
    }
    ...
    try {
      ...
      // Navigate to Cafe Dashboard for this authorized cafe
  ...
  return (
    <div className="w-full space-y-4 animate-fade-in">
      ...
      {selectedCafe ? (
        <div className="space-y-5 bg-[#241A16]/75 p-5 sm:p-7 rounded-3xl border border-[#B98252]/40 backdrop-blur-md shadow-2xl">
          {/* Header & Back Button */}
          ...
          <div className="space-y-3.5 pt-1">
            <div className="flex items-center justify-between">
              ...
              >
                {showPin ? <EyeOff className="w-3.5 h-3.5 text-[#E2A76F]" /> : <Eye className="w-3.5 h-3.5 text-[#DDD4C8]" />}
            ...
            {/* Numeric Keypad for Touchscreens & Desktop */}
        ...
        /* VIEW B: ALL CAFES LIST LOADED FROM POSTGRESQL */
        <div className="space-y-3.5">
          ...
          {isLoading && (
            <div className="py-12 flex flex-col items-center justify-center space-y-3 text-[#DDD4C8]/80">
              ...
              <span className="text-xs sm:text-sm font-medium">Loading partner cafes from database...</span>
            </div>
          )}
          ...
          {!isLoading && !fetchError && (
            <div className="max-h-[min(540px,calc(100vh-320px))] min-h-[320px] overflow-y-auto space-y-2.5 pr-1.5 scrollbar-thin">
                  ...
                  >
                    <div className="flex items-center space-x-3.5 min-w-0 pr-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[#DDD4C8]/75 truncate mt-0.5">{cafe.neighborhood || cafe.address}</p>
                      </div>
                    </div>
          ...
          <div className="pt-1.5 flex items-center justify-between text-[11px] text-[#DDD4C8]/60 font-mono">
            <span>{filteredCafes.length} Roasteries in Database</span>
