...
export const MobileAppContainer: React.FC = () => {
  ...
  return (
    <div className="w-full h-full bg-[#FAF5EF] text-[#241A16] flex flex-col relative select-none">
      {/* Ambient background pattern, confined to the mobile app shell */}
      <div
      ...
      {/* Top Header - fixed mobile app shell width at every viewport size */}
      <header className="sticky top-0 z-30 w-full bg-[#1E1411]/95 backdrop-blur-md border-b border-[#C58A55]/20 text-[#F3E7D5] shadow-md flex-shrink-0">
        <div className="px-4 py-3 flex items-center justify-between gap-4 w-full">
          {/* Brand Logo */}
          <button
          ...
          >
            ...
            <div className="flex items-center space-x-1.5">
              <span className="font-editorial font-bold text-base tracking-wider uppercase text-white">Social Cup</span>
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            <button
              ...
              className="flex items-center space-x-1.5 bg-gradient-to-r from-[#C58A55] to-[#8C4A32] hover:from-[#B37944] hover:to-[#7B3F2A] text-white px-3 py-1.5 rounded-xl text-[11px] font-bold shadow-xs caramel-glow-sm transition-all active:scale-95 cursor-pointer"
            >
            ...
            {liveUser ? (
              <button
                ...
                className="flex items-center space-x-1.5 bg-[#251814] border border-[#C58A55]/30 px-2.5 py-1.5 rounded-xl text-xs font-bold text-[#F3E7D5] hover:border-[#C58A55] transition-all active:scale-95 cursor-pointer"
                title="View Profile & Cupping Journal"
              >
                <div className="w-2 h-2 rounded-full bg-[#537A5A] animate-pulse" />
                <span className="max-w-[110px] truncate">{liveUser.name}</span>
              </button>
            ) : (
              <button
      ...
      {/* Main Viewport - fixed mobile-width content */}
      <main className="w-full flex-1 relative z-10 flex flex-col">
        <div className="w-full px-4 py-5 flex-1 min-w-0 pb-24">
          {activeTab === 'discover' && (
            <DiscoverScreen
      ...
      {/* Bottom Navigation Bar - always visible, part of the mobile app shell */}
      <nav
        className="sticky bottom-0 z-40 bg-[#1E1411]/95 backdrop-blur-md border-t border-[#C58A55]/20 px-6 py-2 flex justify-around items-center shadow-2xl pb-[max(0.5rem,env(safe-area-inset-bottom))]"
